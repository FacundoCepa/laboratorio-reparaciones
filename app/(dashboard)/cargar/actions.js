  ...
  try {
    ...
    if (isStaff) {
      ...
      if (esNuevo) {
        ...
        if (!nombreNuevo || !emailNuevo) return { error: "Faltan datos del cliente nuevo." };
        ...
        if (createErr) {
          if (createErr.code === "email_exists") {
            return { error: `Ese email (${emailNuevo}) ya está registrado. Elegilo de la lista de "cliente existente" en vez de crearlo de nuevo.` };
          }
          return { error: `No se pudo crear el cliente: ${createErr.message}` };
        }
        ...
        if (!clienteExistenteId) return { error: "Elegí un cliente." };
    ...
    console.error("Error inesperado en registrarEquipo:", err);
